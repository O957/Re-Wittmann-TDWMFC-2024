...
def main():
    ...
    init_rhos = [1, 2, 4]  # taxation rates
    ...
    for i, rho in enumerate(init_rhos):
        ...
        N, S = sol.ys.T
        ts = sol.ts
        axes[0].plot(ts.tolist(), N.tolist(), color=rho_colors[i])
        axes[1].plot(ts.tolist(), S.tolist(), color=rho_colors[i])
